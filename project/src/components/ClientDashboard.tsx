...
import { Package, Star, ArrowRight, Check, Calendar, DollarSign, Image, Users } from 'lucide-react';
import { User, EventPackage, CustomizationOption } from '../types';
import { eventPackages, customizationOptions } from '../data/mockData';
...
const ClientDashboard: React.FC<ClientDashboardProps> = ({ user }) => {
  ...
  const getTotalPrice = () => {
    ...
    const customizationTotal = selectedCustomizations.reduce((sum, option) => {
      // For per-person pricing (catering), multiply by guest count
  ...
  const handleBooking = () => {
    ...
    alert('Booking request submitted successfully! You will receive confirmation soon.');
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-2 gap-8">
          {eventPackages.map((pkg) => (
            <div
              key={pkg.id}
              className="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 border hover:border-purple-200"
            >
              {/* Package Image */}
              ...
              <div className="p-6">
                ...
                <div className="flex gap-3">
                  <button
                    ...
                    className="flex-1 bg-gradient-to-r from-purple-600 to-blue-600 text-white px-4 py-3 rounded-xl font-medium hover:from-purple-700 hover:to-blue-700 transition-all duration-200 flex items-center justify-center group"
                  >
                    <span>Customize</span>
                  ...
                  <button
                    ...
                    className="px-6 py-3 border border-purple-200 text-purple-700 rounded-xl font-medium hover:bg-purple-50 transition-colors"
                  >
                    Book Now
                  </button>
                </div>
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-600">
              Base price: ${selectedPackage?.basePrice.toLocaleString()}
            </p>
          </div>
              ...
              <div key={categoryKey} className="bg-white rounded-2xl p-6 shadow-lg">
                ...
                <div className="grid md:grid-cols-3 gap-4">
                  {options.map((option) => {
                    ...
                    const displayPrice = option.category === 'catering' 
                      ? `$${option.price}/person (${guestCount} guests = $${(option.price * guestCount).toLocaleString()})`
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          Review your event details and submit your booking request
        </p>
      </div>
                  ...
                  return (
                    <div key={option.id} className="flex justify-between text-sm">
                      <span className="text-gray-600">
                        {option.name}
                        {option.category === 'catering' && ` (${guestCount} guests)`}
                      </span>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-lg">
          ...
          <div className="space-y-6">
            ...
            <button
              ...
              disabled={!eventDate}
              className="w-full bg-gradient-to-r from-purple-600 to-blue-600 text-white px-6 py-4 rounded-xl font-medium hover:from-purple-700 hover:to-blue-700 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
            >
